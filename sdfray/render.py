...
glsl_render_raytrace = '''    
    vec3 cast_ray_rt(vec3 p, vec3 d) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            bool hit = next_surface(p,d,g);
            if (hit) {
                //seed_rand(p.xy+d.xy+vec2(p.z,d.z)); // avoid periodicity
                Property s = prop(p,d);
                color += prescale*s.emittance;
                vec3 n = normalize(g);
                bool keep_going = false;
                ...
                float rnd = rand();
                if (rnd < s.diffuse) {
                    prescale *= s.diffuse*s.color;
                    vec3 diff = abs(n.x) < 0.5 ? vec3(1.,0.,0.) : vec3(0.,1.,0.);
                    vec3 p1 = cross(n,diff);
                    vec3 p2 = cross(n,p1);
                    float phi = 2.*3.141592658*rand();
                    float costheta = rand();
                    float sintheta = sqrt(1.-costheta*costheta);
                    d = costheta*n + sintheta*(p1*cos(phi)+p2*sin(phi));
                    keep_going = true;
